...
flags = parse_args()                                            # 命令行参数体，内含多个数据配置
cfg = load_config(flags.config_file)

fonts = font_utils.get_font_paths_from_list(flags.fonts_list)   # fonts-字体文件路径组成的列表-目前的文件就1个路径
bgs = utils.load_bgs(flags.bg_dir)                              # 背景图像组成的列表

corpus = corpus_factory(flags.corpus_mode, flags.chars_file, flags.corpus_dir, flags.length)    # 导入语料库-类方法
...
def generate_img(img_index, q=None):
    global flags, lock, counter, img_prefix
    # Make sure different process has different random seed
    np.random.seed()

    im, word = gen_img_retry(renderer, img_index)           # 生成图像和文本

    # base_name = prefix + '{:08d}'.format(img_index)     # 保存图像名，注意加了一个前缀
    base_name = flags.img_prefix + '{:08d}'.format(img_index) + '.jpg'
...
if __name__ == "__main__":
    ...
    # 保存数据相关
    tmp_label_path = os.path.join(flags.save_dir, 'tmp_labels.txt')     # txt保存路径，内容：照片名称-标签
    label_path = os.path.join(flags.save_dir, 'labels.txt')             # txt保存路径, 内容：标签
    start_index = restore_exist_labels(label_path)                      # 标签开始索引：如果文件已存在，则追加-即从下一位开始
    img_prefix = "coltd1_"
